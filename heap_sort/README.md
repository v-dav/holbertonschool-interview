# Heap Sort (Sift-Down) Algorithm

![C](https://img.shields.io/badge/language-C-blue)

This project implements the Heap sort algorithm using the sift-down approach to sort an array of integers in ascending order.

## 🧐 Project Description

The `heap_sort` function takes an array of integers and sorts it using the sift-down Heap sort algorithm. The algorithm repeatedly extracts the maximum element from the heap and reconstructs the heap until the array is sorted.

## 🎓 Constraints

- The function uses the sift-down approach to implement the Heap sort algorithm.
- The array must be of type `int`.
- The size of the array (`size_t size`) should be provided to the `heap_sort` function.
